...
// Creat a new node once we get to the end of the Linked List
...
// hence we reassign pointer resulting in a skipped node i.e. 'deleted node'
            if (current.next.data == data) { // Value identification
                current.next = current.next.next; // Skipping node with value of interest & Pointer reassignment
                return;
            }
            current = current.next;
        }
...
// Loop and print out data at every node
    ...
    public static void main(String [] args){
        LinkedLists prices = new LinkedLists();
        prices.add(20);
        prices.add(100);
        prices.add(150);
        prices.print();
